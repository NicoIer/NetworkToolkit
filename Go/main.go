...
import (
	"fmt"
	"github.com/0990/kcp2k-go/kcp"
	"log"
	"time"
)

func main() {
	if listener, err := kcp.ListenWithOptions("127.0.0.1:12345"); err == nil {
		// spin-up the client
...
func handleEcho(conn *kcp.Session) {
...
func clientSendReliable() {
	...
	if sess, err := kcp.DialWithOptions("127.0.0.1:12345"); err == nil {
		for {
			data := time.Now().String()
			buf := make([]byte, len(data))
			log.Println("sent reliable:", data)
			if _, err := sess.Send([]byte(data), kcp.Reliable); err == nil {
				if err != nil {
					log.Println(err)
					return
				}
...
func clientSendUnreliable() {
	...
	if sess, err := kcp.DialWithOptions("127.0.0.1:12345"); err == nil {
		for {
			...
			sess.Send([]byte(data), kcp.Unreliable)
